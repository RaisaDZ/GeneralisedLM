...
AA_multiclass = function(outcomes, X, M, M0, a, sigma){
  eta = 1
  ...
  calc_probs <- function(X, theta) {
    ...
    ksi <- matrix(0, n, d)
    for (l in 1:d) {
      ksi[, l] <- apply(X * matrix(rep(theta[l, ], n), nrow = n, byrow = T), 1, sum)
    }
    ksi = exp(ksi)
  ...
  for (t in 1:T) {
    ...
    for (m in 2:M)  {
      ...
      theta_new <- theta_old + matrix(rnorm(d*n, 0, sigma^2), nrow = d, ncol = n) #sample new params
      if (t > 1) {
